...
class WillExamineFragment:Fragment(),TaskView{
    ...
    fun getNormalData(){
        //设置内容
        taskPresenter!!.getTaskListApi(TaskListRequest(UserDao.getLocalUser().id,"",pageNumber,10,firstType,1), RefreshAction.NormalAction);
    }
    ...
    override fun onResume() {
        ...
        recyclerview.setOnRefreshListener(object: PullToRefreshBase.OnRefreshListener<ListView>{
            override fun onPullDownToRefresh(refreshView: PullToRefreshBase<ListView>?) {
                ...
                taskPresenter!!.getTaskListApi(
                        TaskListRequest(UserDao.getLocalUser().id,""
                                , pageNumber,10,firstType,1), RefreshAction.PullDownRefresh);
            }
            ...
            override fun onPullUpToRefresh(refreshView: PullToRefreshBase<ListView>?) {
                ...
                taskPresenter!!.getTaskListApi(
                        TaskListRequest(UserDao.getLocalUser().id,"",
                                pageNumber,10,firstType,1), RefreshAction.UpMore);
            }
        })
    }
