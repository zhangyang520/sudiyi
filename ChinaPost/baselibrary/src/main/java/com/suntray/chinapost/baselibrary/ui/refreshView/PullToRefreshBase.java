                ...
                if (isReadyForPullDown()) {
                    ...
                    if (mPullRefreshEnabled && (mPullDownState == ILoadingLayout.State.RELEASE_TO_REFRESH)) {
                        if(mPullUpState==ILoadingLayout.State.REFRESHING){
                        	Toast.makeText(UiUtils.Companion.getInstance().getContext(),refreshTitle+pullDownWhenPullUpError,Toast.LENGTH_SHORT).show();
                        }else{
                        	startRefreshing();
                            handled = true;
                        }
                    ...
                    if (isPullLoadEnabled() && (mPullUpState == ILoadingLayout.State.RELEASE_TO_REFRESH)) {
                        if(mPullDownState==ILoadingLayout.State.REFRESHING){
                        	Toast.makeText(UiUtils.Companion.getInstance().getContext(),refreshTitle+pullUpWhenPullDownError,Toast.LENGTH_SHORT).show();
                        }else{
                        	startLoading();
                            handled = true;
                        }
